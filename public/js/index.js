    ...
    const sendData = data => {
        $.post("/", data).then(function (response) {
            ...
            for (var i = 0; i < response.length; i++) {
                $(".collapsible").prepend("<li><div class='collapsible-header'><img src='" + response[i].logo + "'>" + response[i].title + "<div id='add'>+</div></div><div class='collapsible-body'><p>" + response[i].descript + "</p><a class='urlLink' href='" + response[i].url + "' target='_blank'>" + response[i].url + "</a></div></li>");
            }
        });
    };
